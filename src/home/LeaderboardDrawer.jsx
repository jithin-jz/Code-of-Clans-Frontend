...
import { Trophy, Crown, Medal, Users, Loader2 } from 'lucide-react';
...
const LeaderboardDrawer = ({ isLeaderboardOpen, setLeaderboardOpen }) => {
    ...
    const getRankIcon = (index) => {
        switch (index) {
            case 0: return <Crown size={14} className="text-yellow-500 fill-yellow-500/30" />;
            case 1: return <Medal size={14} className="text-gray-400 fill-gray-400/30" />;
            case 2: return <Medal size={14} className="text-amber-700 fill-amber-700/30" />;
            default: return null;
        }
    };
    ...
    return (
        ...
        >
            <div className="w-full h-full bg-gradient-to-b from-[#0d0d0d] via-[#0a0a0a] to-[#080808] backdrop-blur-3xl border-l border-white/5 flex flex-col pointer-events-auto shadow-2xl shadow-black/50 relative">
                ...
                {/* Header */}
                <div className="relative h-16 border-b border-white/5 flex items-center justify-between px-5 bg-gradient-to-r from-[#141414] to-[#0f0f0f]">
                    {/* Header glow line */}
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <span className="text-white font-bold text-base tracking-tight block leading-tight">Leaderboard</span>
                            <div className="flex items-center gap-1">
                                <Users size={10} className="text-[#FFD700]" />
                                <span className="text-[10px] text-gray-500 uppercase tracking-wider">Top Players</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto px-4 py-4 space-y-2 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-center">
                            ...
                            <p className="text-gray-500 text-sm font-medium">No rankings yet</p>
                            <p className="text-gray-600 text-xs mt-1">Complete challenges to rank!</p>
                        </div>
                    ) : (
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className={`w-9 h-9 rounded-full overflow-hidden ring-2 ${
                                        ...
                                        {rankUser.avatar ? (
                                            <img 
                                                src={`http://localhost:8000${rankUser.avatar}`} 
                                                alt={rankUser.username} 
                                                className="w-full h-full object-cover"
                                            />
                                        ) : (
                                            <div className="w-full h-full bg-gradient-to-br from-blue-500/30 to-purple-500/20 flex items-center justify-center text-xs font-bold text-blue-400">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-[10px] text-gray-500">
                                            {rankUser.completed_levels} levels • {rankUser.xp?.toLocaleString() || 0} XP
                                        </p>
                                    </div>

                                    {/* XP Badge */}
                                    <div className="text-right">
                                        <span className="text-xs font-bold text-amber-500">{rankUser.xp?.toLocaleString() || 0}</span>
                ...
                <div className="p-3 border-t border-white/5 bg-gradient-to-t from-[#0a0a0a] to-transparent">
                    <p className="text-center text-[9px] text-gray-600">
                        Press <kbd className="px-1 py-0.5 bg-white/5 rounded text-gray-500 font-mono">Ctrl+L</kbd> to toggle
                    </p>
                </div>
            </div>
        </motion.div>
